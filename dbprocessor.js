...
setupPilotPage = function (work_dir) {
    if (fs.existsSync(work_dir)) {
        if (fs.existsSync('./resources/pilot.png') && !fs.existsSync(work_dir + "pilot.png")) {
            fs.writeFileSync(work_dir + "pilot.png", fs.readFileSync('./resources/pilot.png'));
        }
        if (fs.existsSync('./resources/util.js')) {
            fs.writeFileSync(work_dir + "util.js", fs.readFileSync('./resources/util.js'));
        }
        if (fs.existsSync('./resources/ifly.css')) {
            fs.writeFileSync(work_dir + "ifly.css", fs.readFileSync('./resources/ifly.css'));
        }
        if (fs.existsSync('./resources/dark-unica.js')) {
            fs.writeFileSync(work_dir + "dark-unica.js", fs.readFileSync('./resources/dark-unica.js'));
        }
        if (fs.existsSync('./resources/dark-unica.js.map')) {
            fs.writeFileSync(work_dir + "dark-unica.js.map", fs.readFileSync('./resources/dark-unica.js.map'));
        }
    }
}

buildUserPad = function (flights, config) {
    pad = ""
    totalSeconds = 0;
    flightCount = 0;
    flightNoIGC = 0;
...
processFlights = function (data, work_dir, config, rowid, dbp, new_etag, email) {
    ...
    data.forEach((row, ii) => {
        ...
        if (flight.hasIGC) {
            ...
            var result_ffvl = solver(score_flight, scoringRules.FFVL).next().value;
            var result_xcontest = solver(score_flight, scoringRules.XContest).next().value;
            ...
            fs.mkdirSync(work_dir + "json/", {
                recursive: true
            });
            fs.writeFileSync(work_dir + "json/" + flight.id + ".js", JSON.stringify(analyzedData.fixes));
            flight.analysed = {
    ...
    template = template.split("aa9f3975e1ac31d104905da5d2fa2d79").join(buildUserPad(flights, config));
    template = template.split("f71dbe52628a3f83a77ab494817525c6").join(JSON.stringify(flights));
    template = template.split("9c2646307b6841b858e16446a494f05a").join(app_config.google_analytics_id);
    ...
    fs.writeFile(work_dir + "index.html", template, {}, (err) => {
        console.log("after");
        // move files to website
        pilot_web_dir = app_config.website_root + "p/" + rowid
        fs.rmdir(pilot_web_dir, {
            recursive: true
        }, (err) => {
            if (err) {
                console.log("thrown")
            ...
            console.log("Pilot's web dir cleaned : " + pilot_web_dir);
            fs.mkdir(pilot_web_dir, {
                recursive: true
            }, (err) => {
                if (err) {
                    console.log("errrrr")
                    ...
                    git_comm += "git add . ;"
                    git_comm += 'git commit -m "' + Date.now() + '" ;'
                    git_comm += "git push ;"
                    git_comm += "cd " + app_config.app_dir
                    // git push
                    exec(git_comm, (err, stdout, stderr) => {
                        if (err) {
                            console.error(err)
                        } else {
                            console.log(stdout);
                            console.log("Pilot's page pushed");
                            // update etag to db
                            ...
                            dbp.close((err) => {
                                ...
                                console.log('Close the pilots database connection.');
...
// open the database
function processdb(work_dir, config, dbp, rowid, new_etag, email) {
    setupPilotPage(work_dir)
    const db = new sqlite3.Database(work_dir + config.logFly_file, sqlite3.OPEN_READONLY, (err) => {
        if (err) {
            console.error(err.message);
            console.log(work_dir + config.logFly_file)
        }
    ...
    db.serialize(() => {
        db.all(`SELECT * FROM Vol order by V_Date ASC`, (err, rows) => {
            ...
            processFlights(rows, work_dir, config, rowid, dbp, new_etag, email);
    ...
    db.close((err) => {
        ...
        console.log('Close the flights database connection.');
        //console.log(flights_table);
    });
}
